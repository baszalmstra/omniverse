...
impl Node {
    pub fn new(backing: &mut NodeBacking, geometry: &planet::PatchGeometry) -> Node {
        use planet::constants::{VERTICES_PER_PATCH, NORMALS_PER_PATCH};
        ...
        unsafe {
            ...
            for (i, pos) in geometry
                ...
                .enumerate()
                {
                    ...
                    let morph_target_index = i -
                        ((x % 2) * 1) -
                        ((y % 2) * VERTICES_PER_PATCH);
                    ...
                    vertices[i].local_texcoords = [x as f32 / (VERTICES_PER_PATCH - 1) as f32, y as f32 / (VERTICES_PER_PATCH - 1) as f32];
                }

            for (i, normal) in geometry
                .normals
                .iter()
                .enumerate()
                {
