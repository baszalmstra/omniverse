use nalgebra::{Point3, Vector3, Real, Scalar};
...
impl<T:Real> Cone<T> {
    ...
    pub fn new(camera_position:Point3<T>, planet_radius:T) -> Cone<T> {
        ...
        let cos_angle = near_distance / (distance_to_center*distance_to_center - radius_squared).sqrt();
        Cone {
    ...
    where N:SubsetOf<T>
{
    fn classify(&self, shape: &Point3<N>) -> Containment {
        ...
        if point_cos_angle > self.near_distance && point_cos_angle/position_camera.norm() > self.cos_angle {
            Containment::Inside
        } else {
            Containment::Outside
        }
