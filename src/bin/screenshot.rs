...
fn main() {
    ...
    use omniverse::transform::{Transform, Rotation, Transformable};
    ...
    let screenshot_infos:Screenshots = serde_json::from_str(&screenshots_content)
        .expect("Could not parse screenshots.json");
    ...
    for screenshot in screenshot_infos.screenshots.iter() {
        ...
        let size = screenshot.size.or(screenshot_infos.size).unwrap_or((1280,800));

        let frame_buffer_size = (size.0 * samples as u32, size.1 * samples as u32);
        let render_texture = glium::texture::SrgbTexture2d::empty(&display, frame_buffer_size.0, frame_buffer_size.1).unwrap();
        let depth_stencil_target = glium::texture::DepthTexture2d::empty(&display, frame_buffer_size.0, frame_buffer_size.1).unwrap();

        let mut frame_buffer = glium::framebuffer::SimpleFrameBuffer::with_depth_buffer(&display,
                                                                                        render_texture.main_level(),
                                                                                        depth_stencil_target.main_level()).unwrap();

        // Initialize a camera
        let mut camera = Camera::new();
        camera.translate_by(&Vector3::new(screenshot.position.0, screenshot.position.1, screenshot.position.2));
        camera.rotate_by(&Rotation::from_euler_angles(screenshot.rotation.0.to_radians(), screenshot.rotation.1.to_radians(), screenshot.rotation.2.to_radians()));
        ...
        while image.width() > size.0 {
            image = image.resize(image.width() >> 1, image.height() >> 1, image::FilterType::Triangle)
        }
...
impl CanGammaCorrect for (f32,f32,f32) {
    fn from_linear(self) -> Self {
        (self.0.powf(GAMMA_EXP), self.1.powf(GAMMA_EXP), self.2.powf(GAMMA_EXP))
    }
}

impl CanGammaCorrect for (f32,f32,f32, f32) {
    fn from_linear(self) -> Self {
        (self.0.powf(GAMMA_EXP), self.1.powf(GAMMA_EXP), self.2.powf(GAMMA_EXP), self.3)
    }
}